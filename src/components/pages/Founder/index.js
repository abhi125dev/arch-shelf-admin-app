...
const Founder = ({ user, feeds, getFeedsFunc }) => {
  const { Search } = Input;
  ...
  useEffect(() => {
    getOptions({ query: { categoryType: "founder" } }).then((res) => {
  ...
  return (
    <div className="content-panel">
      <PageMetaTags title="Founder" />
      <Breadcrumb style={{ marginBottom: 20 }}>
        <Breadcrumb.Item>
          <Link to="/dashboard">Home</Link>
        </Breadcrumb.Item>
        <Breadcrumb.Item>Founder</Breadcrumb.Item>
      </Breadcrumb>
      <div className="flex items-center justify-between space-x-6">
        <span className="page-heading">Founder</span>
        <Input
          size="small"
        ...
        <div className="flex justify-end space-x-4">
          ...
          <Button
            size="large"
            type="primary"
            onClick={() => history.push("/founders/add")}
          >
            Add article
          </Button>
        </div>
      ...
      {feedsList && feedsList.feedList && feedsList.feedList.length > 0 ? (
        <Spin tip="Loading..." spinning={loading} size="large">
          {feedsList &&
            feedsList.feedList.map((val) => <Card type="founder" item={val} />)}
...
Founder.propTypes = {
  Founder: PropTypes.func.isRequired,
