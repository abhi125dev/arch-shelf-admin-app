...
const Courses = ({ user, feeds, getFeedsFunc }) => {
  const { Search } = Input;
  ...
  useEffect(() => {
    getOptions({ query: { categoryType: "courses" } }).then((res) => {
    ...
    <div className="content-panel">
      ...
      {feedsList && feedsList.feedList && feedsList.feedList.length > 0 ? (
        <Spin tip="Loading..." spinning={loading} size="large">
          {feedsList &&
            feedsList.feedList.map((val) => <Card type="courses" item={val} />)}
...
Courses.propTypes = {
  GHCAcademy: PropTypes.func.isRequired,
